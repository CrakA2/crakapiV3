...
import { calculateWinLoss } from '$lib/server/session';
...
export const GET: RequestHandler = async ({ params, url }) => {
  ...
  const cacheKey = cache.buildKey('wl', region, puuid, platform, compOnly ? 'comp' : 'all');
  ...
  try {
    ...
    const result = calculateWinLoss(matches, compOnly);
