...
import { calculateKDA } from '$lib/server/session';
...
export const GET: RequestHandler = async ({ params, url }) => {
  ...
  const cacheKey = cache.buildKey('kd', region, puuid, platform, compOnly ? 'comp' : 'all');
  ...
  try {
    ...
    const result = calculateKDA(matches, compOnly);
